            ...
            if (input_y < input_height) {
              for (size_t kernel_x = 0; kernel_x < kernel_width; kernel_x++) {
                ...
                if (input_x < input_width) {
                  indirection_buffer[index] = (uint8_t*)input + ((image * input_height + input_y) * input_width + input_x) * input_pixel_stride + group * group_input_channels;
                } else {
        ...
        if (input_y < input_height) {
          for (size_t output_x = 0; output_x < output_width; output_x++) {
            for (size_t kernel_x = 0; kernel_x < kernel_width; kernel_x++) {
              ...
              if (input_x < input_width) {
                indirection_buffer[index] = (uint8_t*)input + ((image * input_height + input_y) * input_width + input_x) * input_pixel_stride;
              } else {
                indirection_buffer[index] = zero;
              }
            }
            ...
            for (size_t kernel_x = 0; kernel_x < kernel_width; kernel_x++) {
              ...
              if (input_y * stride_height == y && input_y < input_height && input_x * stride_width == x && input_x < input_width) {
                indirection_buffer[index] =
                  (uint8_t*)input + ((image * input_height + input_y) * input_width + input_x) * input_pixel_stride + group * group_input_channels;
              } else {
                indirection_buffer[index] = zero;
              }
            }
      ...
      for (size_t pooling_y = 0; pooling_y < pooling_height; pooling_y++) {
        ...
        for (size_t output_x = 0; output_x < output_width; output_x++) {
          for (size_t pooling_x = 0; pooling_x < pooling_width; pooling_x++) {
            ...
            indirection_buffer[index] = (uint8_t*)input + ((image * input_height + clamped_input_y) * input_width + clamped_input_x) * input_pixel_stride;
          }
        }
      }
    }
