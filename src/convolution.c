      ...
      switch (kernel_size) {
        ...
        case 25:
          ...
          pack_q8dw_w_dilation(
            ...
            groups, cr,
            0, kernel_height, 2, 4,
            kernel, bias, (uint8_t*)convolution->packed_weights + (10 + sizeof(int32_t) / sizeof(uint8_t)) * c_stride, false);
          pack_q8dw_w_dilation(
            kernel_height, kernel_width,
            groups, cr,
            0, kernel_height, 4, 5,
            kernel, bias, (uint8_t*)convolution->packed_weights + (20 + sizeof(int32_t) / sizeof(uint8_t)) * c_stride, false);
          break;
        default:
          QNNP_UNREACHABLE;
      }
