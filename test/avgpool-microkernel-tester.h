 ...
 public:
  ...
  void test(q8avgpool_up_ukernel_function q8avgpool) const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const union qnnp_avgpool_quantization_params quantizationParams =
        qnnp_compute_avgpool_quantization_params(
          -int32_t(xZeroPoint()) * int32_t(ks()),
          xScale() / (yScale() * float(ks())),
          yZeroPoint(), yMin(), yMax());
      const union qnnp_avgpool_quantization_params scalarQuantizationParams =
        qnnp_compute_scalar_avgpool_quantization_params(
          -int32_t(xZeroPoint()) * int32_t(ks()),
          xScale() / (yScale() * float(ks())),
          yZeroPoint(), yMin(), yMax());
      ...
      for (size_t i = 0; i < n(); i++) {
        for (size_t k = 0; k < kc(); k++) {
          ...
          yAcc[i * kc() + k] = acc;
          yRef[i * kc() + k] = qnnp_avgpool_quantize(acc, scalarQuantizationParams);
          yFP[i * kc() + k] = float(acc) * (xScale() / (yScale() * float(ks()))) + float(yZeroPoint());
      ...
      q8avgpool(n(), ks(), kc(),
        indirectX.data(), zero.data(), y.data(),
        kh() * s() * sizeof(void*),
      ...
      for (size_t i = 0; i < n(); i++) {
        for (size_t k = 0; k < kc(); k++) {
          ...
          ASSERT_GE(uint32_t(y[i * yStride() + k]), uint32_t(yMin()))
            << "at pixel " << i << ", channel " << k << ", n = " << n() << ", kc = " << kc();
          ASSERT_NEAR(float(int32_t(y[i * yStride() + k])), yFP[i * kc() + k], 0.5f)
            << "at pixel " << i << ", channel " << k << ", n = " << n()
            << ", ks = " << kh() << "x" << kw() << " (" << ks() << "), kc = " << kc()
            << ", acc = " << yAcc[i * kc() + k];
          ASSERT_EQ(uint32_t(yRef[i * kc() + k]), uint32_t(y[i * yStride() + k]))
            << "at pixel " << i << ", channel " << k << ", n = " << n()
            << ", ks = " << kh() << "x" << kw() << " (" << ks() << "), kc = " << kc()
            << ", acc = " << yAcc[i * kc() + k];
        }
