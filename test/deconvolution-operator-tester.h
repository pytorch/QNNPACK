...
class DeconvolutionOperatorTester {
 public:
  ...
  inline size_t outputHeight() const {
    return strideHeight() * (inputHeight() - 1) + adjustmentHeight() + dilatedKernelHeight() - paddingHeight();
  }
  ...
  void testQ8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
                      ...
                      for (size_t oc = 0; oc < groupOutputChannels(); oc++) {
                        for (size_t ic = 0; ic < groupInputChannels(); ic++) {
                          accumulators[(((i * outputHeight() + oy) * outputWidth() + ox) * groups() + g) * groupOutputChannels() + oc] +=
                            ...
                            (int32_t(kernel[(((g * groupInputChannels() + ic) * kernelHeight() + ky) * kernelWidth() + kx) * groupOutputChannels() + oc]) - int32_t(kernelZeroPoint));
                        }
                      }
                    }
                  }
      ...
      qnnp_operator_t deconvolution = nullptr;


      ASSERT_EQ(qnnp_status_success,
        qnnp_create_deconvolution2d_nhwc_q8(
          paddingTop(), paddingRight(), paddingBottom(), paddingLeft(),
          adjustmentHeight(), adjustmentWidth(),
      ...
      ASSERT_EQ(qnnp_status_success,
        qnnp_run_operator(deconvolution, nullptr /* thread pool */));
          ...
          for (size_t x = 0; x < outputWidth(); x++) {
            for (size_t g = 0; g < groups(); g++) {
              for (size_t c = 0; c < groupOutputChannels(); c++) {
                ...
                ASSERT_LE(
                  double(int32_t(output[((i * outputHeight() + y) * outputWidth() + x) * outputPixelStride() + g * groupOutputChannels() + c]) - outputZeroPoint),
                  double(qmax()) - double(outputZeroPoint))
